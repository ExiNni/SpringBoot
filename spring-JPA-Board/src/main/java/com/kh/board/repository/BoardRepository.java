...
/*
	...
	%:keyword% - keyword 파라미터로 받아온 키워드를 나타냄 %어떤 문자열이라도 매칭이 될 수 있도록 도와주는 역할을 함
	List<Board>: 검색 결과를 리스트 형태로 반환할 수 있도록 해줌
	
	@Param("keyword"): keyword에 해당하는 값을 매서드의 파라미터로 받아오기 위해 
					   @Param 이라는 어노테이션을 사용, 메서드에서 매개변수로 전달된 keyword의 값을 쿼리 내에 :keyword에 매핑시킴
*/
...
/*
	...
	Board b / b를 붙이는 것과 안 붙이는 것은 엔티티에서 별칭을 지정해서 사용하는 방식
