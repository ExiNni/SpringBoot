...
public class LoginController {
	...
	public String login(String mname, String memail, Model model) {
		...
		if(userInfo != null) { // 만약에 로그인을 성공하게 된다면 모델에 loginSuccess를 활용해서 성공여부를 추가
							   // 필수로 필요한 것은 아니며 추가를 하지 않아도 됨
			model.addAttribute("loginSuccess", true);
			return "loginSuccess";
		} else {
			// 로그인을 실패하면 error 속석을 모델에 추가해서 로그인에 실패했습니다 라는 메세지를 모델에 담을 수 있도록 진행함
			model.addAttribute("error", "로그인에 실패했습니다");
			// 로그인 실패 후 다시 로그인을 하기 위해 /user/login 페이지로 이동할 수 있도록 지정해줌
			return "login";
		}
